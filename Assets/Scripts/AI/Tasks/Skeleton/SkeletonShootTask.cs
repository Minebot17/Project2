	...
	public override bool Handle() {
		...
		if (!result.IsCancel) {
			...
			Timer.StartNewTimer("ShootBoneRender", 0.4f, 1, gameObject, timer => {
				...
				bone.GetComponent<Rigidbody2D>().AddForce(vector*(float)(InitScane.rnd.NextDouble()/10f + 0.9f)*30000 + new Vector2(0, 50));
				bone.GetComponent<Rigidbody2D>().AddTorque(30000f * (float)InitScane.rnd.NextDouble());
